...
int main(int narg, char *argv[])
{
    if(narg != 4)
    {
        cout << "Usage: " << argv[0] << " <ip> <port> <filepath>" << endl;
        return -1;
    }
    ...
    }
    {
        ...
        if (ret != 0)
        {
            std::cout << "ERROR: connecting to server: " << argv[1] << ":" << argv[2] << std::endl;
            return -1;
        }
        std::shared_ptr<int> x(NULL, [&](int *){close(sockfd);});
        StreamControl stream_control(&hwrdma, sockfd, &local_conf);
        ...
        std::cout << "Sending file: " << argv[3] << std::endl;
        char filename[256];
        if(parse_filename(argv[3], filename) < 0)
        {
            cout << "ERROR: parsing filename failed." << endl;
            return -1;
        }
        if(stream_control.postSendFile(argv[3], filename) < 0)
        {
